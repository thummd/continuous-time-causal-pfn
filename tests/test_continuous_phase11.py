...
Covers the ``num_substeps`` knob on :meth:`ContinuousSCM.simulate` that
realises tier-(C) continuous integration from Section 3.1 of the paper:
each observation gap is split into ``num_substeps`` Euler-Maruyama
steps, with independent noise per sub-step, so the trajectory
approximates the true SDE law as ``num_substeps -> infinity``.
"""
...
def test_num_substeps_defaults_to_one_and_preserves_behaviour():
    """num_substeps=1 must reproduce pre-Phase-11 behaviour exactly."""
    scm = _mk_scm_ou()
    ...
    _, x2 = scm.simulate(times, dts, generator=g2, num_substeps=1)
    assert torch.equal(x1, x2)


def test_num_substeps_rejects_bad_values():
    scm = _mk_scm_ou()
    times, dts = regular_schedule(T=5, dt=0.5)
    with pytest.raises(ValueError):
        scm.simulate(times, dts, num_substeps=0)
    with pytest.raises(ValueError):
        scm.simulate(times, dts, num_substeps=-2)
    with pytest.raises(ValueError):
        scm.simulate(times, dts, num_substeps=1.5)  # must be int


def test_noise_shape_scales_with_num_substeps():
    """The caller-facing contract: noise must be
    (T-1)*num_substeps x n_vars."""
    scm = _mk_scm_ou()
    times, dts = regular_schedule(T=5, dt=0.5)
    ...
    scm.simulate(times, dts, noise=noise, num_substeps=3)

    # Old shape (T-1, n_vars) rejected when num_substeps > 1.
    with pytest.raises(ValueError):
        scm.simulate(times, dts, noise=torch.zeros(4, 1), num_substeps=3)
...
def test_output_shape_is_observation_grid_regardless_of_substeps():
    ...
    for k in (1, 2, 5, 10, 25):
        _, traj = scm.simulate(
            times, dts, generator=torch.Generator().manual_seed(k), num_substeps=k,
        )
        assert traj.shape == (20, 1), f"wrong shape at num_substeps={k}"
...
def test_fine_grid_converges_to_true_ou_variance(theta, sigma, horizon):
    """As num_substeps increases, the Monte Carlo variance at the
    horizon converges to the OU stationary variance.  num_substeps=1
    systematically over-estimates the variance (Euler-Maruyama bias
    at coarse dt), and num_substeps >> 1 tightens around the truth.

    Quantitative comparison:

    - Naive one-step EM (num_substeps=1, dt = horizon): its stationary
    ...
    - Fine-grid EM (num_substeps >> 1, dt = horizon / num_substeps):
    ...
    def mc_variance(num_substeps, n_traj, seed):
        xs = torch.empty(n_traj)
        g = torch.Generator().manual_seed(seed)
        for j in range(n_traj):
            _, traj = scm.simulate(times, dts, generator=g, num_substeps=num_substeps)
            xs[j] = traj[-1, 0]
        return float(xs.var(unbiased=True).item())

    target = _ou_theoretical_stationary_var(theta, sigma)
    v_coarse = mc_variance(num_substeps=1, n_traj=4000, seed=0)
    v_fine = mc_variance(num_substeps=200, n_traj=2000, seed=1)
...
def test_fine_grid_trajectory_law_is_approximately_schedule_invariant():
    ...
    def mc_var_at_5(times, dts, num_substeps, n_traj=3000, seed=0):
        xs = torch.empty(n_traj)
        g = torch.Generator().manual_seed(seed)
        for j in range(n_traj):
            _, traj = scm.simulate(times, dts, generator=g, num_substeps=num_substeps)
    ...
    v_b_sparse = mc_var_at_5(times_sparse, dts_sparse, num_substeps=1, seed=0)
    v_b_dense = mc_var_at_5(times_dense, dts_dense, num_substeps=1, seed=1)
    ...
    v_c_sparse = mc_var_at_5(times_sparse, dts_sparse, num_substeps=64, seed=2)
    v_c_dense = mc_var_at_5(times_dense, dts_dense, num_substeps=32, seed=3)
...
def test_counterfactual_pair_preserves_pre_intervention_match_at_fine_grid():
    ...
    pre-intervention, at any num_substeps."""
    ...
    for k in (1, 4, 16):
        ...
        _, x_obs, x_cf = scm.sample_counterfactual_pair(
            times, dts, intervention=intervention, generator=g, num_substeps=k,
        )
        ...
        assert torch.allclose(x_obs[pre_mask], x_cf[pre_mask], atol=1e-6), (
            f"pre-intervention mismatch at num_substeps={k}"
        )
